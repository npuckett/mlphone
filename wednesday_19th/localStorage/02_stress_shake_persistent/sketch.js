// ==============================================
// CHARACTER CONTROLLER: SHAKE DETECTION + LOCAL STORAGE
// ==============================================
// This example demonstrates how DEVICE SHAKE affects a parameter
// AND how to use localStorage to persist the stress value between visits.
// 
// CONCEPT: "Persistent Shake Stress"
// - Shaking the device INCREASES stress (negative effect)
// - Keeping device still allows stress recovery
// - Stress value PERSISTS when you refresh the page
// - Character remembers its stress level from last session
//
// PATTERN: INPUT → PARAMETER → OUTPUT + STORAGE
// - INPUT: deviceShaken() sensor
// - PARAMETER: stress (0-100) - SAVED to localStorage
// - OUTPUT: Color shift, position jitter, movement speed
// - STORAGE: Automatic save/load of stress value
//
// ==============================================
...
// ==============================================
// PARAMETER: Stress Level (PERSISTED)
// ==============================================
let stress = 0;  // Current stress level (0 = calm, 100 = maximum stress)
                 // This value will be saved and loaded from localStorage

// Parameter configuration
const STRESS_SHAKE_INCREASE = 8;    // How much stress each shake adds
...
function setup() {
  ...
  // ==========================================
  // LOCAL STORAGE: LOAD stress value
  // ==========================================
  // Load the saved stress value BEFORE creating the character
  ...
  // ==========================================
  // LOCAL STORAGE: Create reset button
  // ==========================================
  createResetButton();
}
...
function draw() {
  ...
  // ==========================================
  // LOCAL STORAGE: SAVE stress value
  // ==========================================
  // Save stress value every frame (or you could save less frequently)
  saveStressToStorage();
...
// LOAD stress value from localStorage
function loadStressFromStorage() {
  // Get the saved stress value
  let savedStress = localStorage.getItem('characterStress');
  
  // If a value exists, use it
  if (savedStress !== null) {
    stress = Number(savedStress);
    displayStress = stress;  // Also set displayStress to avoid sudden jumps
    console.log('📦 Loaded stress from storage:', stress.toFixed(1));
  } else {
    // First visit - stress starts at 0
    console.log('🆕 First visit - stress starts at 0');
  }
}

// SAVE stress value to localStorage
function saveStressToStorage() {
  // Save the current stress value
  localStorage.setItem('characterStress', stress);
  
  // Optional: Only save when stress changes significantly to reduce writes
  // This is already efficient enough for this example
}

// RESET stress to zero and clear storage
function resetStress() {
  // Remove saved value from localStorage
...
function createResetButton() {
  ...
  resetButton.mousePressed(resetStress);
...
function drawUI() {
  ...
  textAlign(LEFT, TOP);
  textSize(16);
  text('⚡ PERSISTENT SHAKE STRESS', x, y);
  y += lineHeight * 1.5;
  ...
  textSize(12);
  let savedStress = localStorage.getItem('characterStress');
  text(`  Saved Value: ${savedStress !== null ? Number(savedStress).toFixed(1) : 'none'}`, x, y);
  y += lineHeight;
